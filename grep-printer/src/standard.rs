...
impl StandardBuilder {
    ...
    /// Build printer using any implementation of `termcolor::WriteColor`.
    ///
    ...
    /// [`Standard::stats`](struct.Standard.html#method.stats)
    /// method.
    ///
...
impl<W: WriteColor> Standard<W> {
    ...
    /// exist. The stats returned are cumulative over all searches performed
    /// using this printer.
...
impl<'a, M: Matcher, W: WriteColor> StandardImpl<'a, M, W> {
    ...
    fn write_line_term(&self) -> io::Result<()> {
        match self.searcher.line_terminator() {
            LineTerminator::Byte(b) => self.write(&[b]),
            LineTerminator::CRLF => self.write(&[b'\r', b'\n']),
        }
    }
